      ...
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex justify-center items-center p-4">
          ...
          <motion.div 
            ...
            className="bg-white w-full max-w-2xl h-auto max-h-[90vh] shadow-2xl rounded-[24px] md:rounded-[32px] relative z-10 flex flex-col border border-white/20 overflow-hidden"
          >
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-[#f1f5f9] bg-[#f8fafc]">
              <h2 className="font-bold text-lg md:text-xl text-[#1a273b]">Settings & Profile</h2>
              <button onClick={onClose} className="p-2 hover:bg-[#ebe9f2] rounded-xl text-[#6b6684] transition-colors"><X className="w-6 h-6" /></button>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 md:p-6 subtle-scroll space-y-8">
            ...
            <div className="p-4 md:p-6 border-t border-[#f1f5f9] bg-[#f8fafc]">
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex justify-center items-center p-4">
          ...
          <motion.div 
            ...
            className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto subtle-scroll rounded-[24px] md:rounded-[32px] shadow-2xl relative z-10 flex flex-col border border-white/20 p-5 md:p-8"
          >
             <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 hover:bg-[#ebe9f2] rounded-full text-[#6b6684] transition-colors bg-white z-10 shadow-sm"><X className="w-6 h-6" /></button>
             
             <div className="mb-8">
               <h2 className="text-xl md:text-2xl font-bold text-[#1a273b] mb-2 tracking-tight">Personalization Settings</h2>
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex justify-center items-center p-4">
          ...
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 300 }}
            className="bg-white w-full max-w-5xl max-h-[90vh] overflow-y-auto subtle-scroll rounded-[24px] md:rounded-[32px] shadow-2xl relative z-10 flex flex-col border border-white/20 p-5 md:p-8"
          >
             <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 hover:bg-[#ebe9f2] rounded-full text-[#6b6684] transition-colors bg-white z-10 shadow-sm"><X className="w-6 h-6" /></button>
             
             <div className="text-center mb-8 md:mb-10 mt-4">
               <h2 className="text-2xl md:text-3xl font-bold text-[#1a273b] mb-3 tracking-tight">Level up your tax workflows</h2>
               <p className="text-base md:text-lg text-[#53637a] max-w-2xl mx-auto font-medium">Choose a plan that scales with your CA firm. Upgrade anytime.</p>
             </div>
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex justify-center items-center p-4">
          ...
          >
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-[#f1f5f9] bg-[#f8fafc]">
              <h2 className="font-bold text-lg md:text-xl text-[#1a273b]">Help & Support</h2>
              <button onClick={onClose} className="p-2 hover:bg-[#ebe9f2] rounded-xl text-[#6b6684] transition-colors"><X className="w-6 h-6" /></button>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 md:p-6 subtle-scroll space-y-6">
...
const LogoutWidget = ({ onOpenDialog }: { onOpenDialog: () => void }) => (
  <motion.button
    ...
    className="fixed bottom-6 right-4 md:bottom-10 md:right-10 w-14 h-14 md:w-16 md:h-16 bg-white shadow-2xl rounded-2xl flex items-center justify-center border border-[#e2e8f0] z-[100] cursor-pointer"
  >
    <div className="relative">
      <Sparkles className="w-7 h-7 md:w-8 md:h-8 text-[#3A759B]" />
    ...
    return (
      <div className="flex h-screen w-full bg-[#f8fafc] items-center justify-center relative overflow-hidden font-sans">
        ...
        <motion.div 
          ...
          className="text-center z-10 p-6 md:p-8"
        >
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-[#1a273b] mb-4">Session Terminated</h1>
          <p className="text-base md:text-lg text-[#53637a] mb-8 max-w-md">Your workspace is now inactive. Secure logout complete.</p>
          <button 
            onClick={() => window.location.reload()}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: "spring", bounce: 0, duration: 0.4 }} className="md:hidden fixed inset-y-0 left-0 z-50 w-[85vw] max-w-[320px] shadow-[10px_0_40px_rgba(0,0,0,0.08)] bg-white/90 backdrop-blur-2xl border-r border-white">
              <Sidebar 
                onNewChat={handleNewChat} 
